...
For declarative shell views, use `edge` for docked chrome, `fill = true` for content, `parent` for controls inside containers, `axis` for row or column child flow, and min/max size fields when a surface needs resize constraints. The runtime keeps shell views bound to the window and reapplies their frames on resize.
...
`ViewInfo.focused` reports the last successfully focused view in a window. Automation snapshots include the same focus state for native controls and WebView-backed views.
