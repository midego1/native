...
.{
    ...
    .platforms = .{ "macos" },
    .permissions = .{ "window", "clipboard", "credentials" },
    .capabilities = .{ "webview", "js_bridge", "clipboard", "credentials" },
    .bridge = .{
        .commands = .{
    ...
    .windows = .{
        .{
            ...
            .views = .{
                ...
                .{ .label = "sidebar", .kind = "sidebar", .edge = "left", .width = 280, .min_width = 220, .max_width = 360 },
                .{ .label = "sidebar-stack", .kind = "stack", .parent = "sidebar", .x = 16, .y = 16, .width = 220, .height = 120, .axis = "column" },
                .{ .label = "sidebar-live", .kind = "checkbox", .parent = "sidebar-stack", .text = "Live updates" },
...
Supported `kind` values are `webview`, `toolbar`, `titlebar_accessory`, `sidebar`, `statusbar`, `split`, `stack`, `button`, `icon_button`, `checkbox`, `toggle`, `segmented_control`, `text_field`, `search_field`, `label`, `spacer`, `gpu_surface`, and `progress_indicator`.

Each view has a required `label` and `kind`. WebView views require `url`. Optional layout fields are `parent`, `edge`, `axis`, `x`, `y`, `width`, `height`, `min_width`, `min_height`, `max_width`, `max_height`, `fill`, and `layer`. Optional behavior and accessibility metadata are `visible`, `enabled`, `role`, `text`, and `command`. The `axis` field accepts `row` or `column` on parent containers such as `toolbar`, `sidebar`, and `stack`; it defaults to `row`.

`createShellWindow` and `createShellViews` dock `edge` views against the remaining window content, let one or more `fill` views use the final remaining rectangle, clamp resolved frames with any min/max size fields, and lay out parented controls such as toolbar buttons with small native defaults when explicit `x`, `y`, `width`, or `height` values are omitted. Parent containers flow omitted child positions horizontally with `axis = "row"` and vertically with `axis = "column"`. The runtime keeps the shell view slice as a layout binding and reapplies it when the window is resized, so pass data that lives for the lifetime of the window.
...
Linux Chromium builds currently reject non-empty `shortcuts`; use the Linux system WebView backend when an app needs native shortcut events.
